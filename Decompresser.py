...
def traiterFichier(bitsDuFichier, nomFichier, bavard):
    ...
    :return: 
    """
    listeBits,listeCaracs = separerCarac(bitsDuFichier) # bitsDuFichier ne contient plus que arbre + texte apres cette ligne
...
def separerCarac(listeBits):
    ...
    while(not finCarac):
        ...
        if(listeCarac.count(int(caracActuel,2)) !=0 and int(caracActuel,2) != listeCarac[0]):#Si le carac actuel existe deja et que ce n'est pas le premier, car le premier est le rang du carac de fin de caractere
            finCarac = True
        else:
