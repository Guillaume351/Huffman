...
def creerArbre(dictCarac):
    ...
    while (len(liste_de_feuilles_disponibles) > 1):  # Tant que toutes les feuilles ne sont pas associées
        somme_branches = liste_de_feuilles_disponibles[-1][0] + liste_de_feuilles_disponibles[0][
            0]  # On prend somme des poids de deux branches au hasard
        mini1, mini2 = -1, 0
        for i in range(len(
                liste_de_feuilles_disponibles)):  # On cherche la combinaison de branche ayant le poids le plus faible
        ...
        liste_de_feuilles_disponibles.remove(
            liste_de_feuilles_disponibles[max(mini1, mini2)])  # On retire les listes qui sont associées
        liste_de_feuilles_disponibles.remove(liste_de_feuilles_disponibles[min(mini1, mini2)])
        liste_de_feuilles_disponibles.append([liste1[0] + liste2[0], liste1 if (liste1[0] < liste2[0]) else liste2,
                                              liste2 if (liste1[0] < liste2[
                                                  0]) else liste1])  # On ajoute la somme des poids devant,On met la feuille la plus faible à gauche
...
def afficher_arbre(arbre, cheminParcouru, texte):
    ...
    for k in range(len(cheminParcouru) - 1):
        if ((cheminParcouru[k] == 1) and (
                (cheminParcouru[k:].count(2) != cheminParcouru[k:].count(1)) or len(cheminParcouru[k:]) <= 2)):
...
def etablirTable(arbre, cheminParcouru=[1], table=dict()):
    ...
    if (len(endroitActuel) != 2):  # Si on est pas dans une feuille
        ...
        if (cheminParcouru.count(
                1) == 0):  # Si on est totalement à droite et que l'on est sur une feuille(count 1 nous dit le nombre de fois que l'on a prit a gauche)
            return table
